...
const Collection = () => {
  ...
  const subCategories = {
    Bracelets: [],
    Earrings: {
      default: ['Chandbali', 'Drops', 'Hoops', 'Danglers', 'Jhumkas', 'Korean', 'Oxidised', 'Studs', 'Sui Dhaga'],
      Chandbali: ['AD', 'Kundan', 'Light Weight', 'Meenakari'],
      Hoops: ['AD', 'Kundan', 'Light Weight', 'Meenakari'],
      Danglers: ['AD', 'Kundan', 'Light Weight', 'Meenakari', 'Anti Tarnish', 'Pearls'],
      Drops: ['AD', 'Kundan', 'Light Weight', 'Meenakari', 'Anti Tarnish', 'Pearls'],
      Jhumkas: ['AD', 'Kundan', 'Meenakari'],
      Studs: ['AD', 'Kundan', 'Light Weight', 'Meenakari', 'Anti Tarnish', 'Pearls'],
    },
    Jewellery: [],
    Necklace: [],
    Rings: {
      default: ['AD', 'Anti Tarnish', 'Cocktail', 'Kundan and Meenakari', 'Oxidised']
    }
  };
  ...
  const applyFilter = () => {
    ...
    if (subSubCategory.length > 0) {
      productsCopy = productsCopy.filter(item => subSubCategory.includes(item.subSubCategory));
    }
        ...
        {category.length > 0 && (
          <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
            ...
            <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
              {category.map((cat) => (
                <div key={cat}>
                  {subCategories[cat].default && subCategories[cat].default.length > 0 && (
                    <>
                      <p className='flex gap-2'>
                        <input className='w-3' type="checkbox" value={cat} onChange={toggleSubCategory} /> {cat}
                      </p>
          ...
          <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
            ...
            <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
              {subCategory.map((subCat) => (
                <div key={subCat}>
                  {subCategories[category.find(cat => subCategories[cat][subCat])][subCat] && (
                    <>
                      <p className='flex gap-2'>
                        <input className='w-3' type="checkbox" value={subCat} onChange={toggleSubCategory} /> {subCat}
                      </p>
                      {subCategories[category.find(cat => subCategories[cat][subCat])][subCat].map((subSub) => (
                        <p className='flex gap-2 pl-5' key={subSub}>
                          <input className='w-3' type="checkbox" value={subSub} onChange={toggleSubSubCategory} /> {subSub}
                        </p>
                      ))}
                    </>
                  )}
